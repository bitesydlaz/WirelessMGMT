...
The __Wireless Management IP__ is __10.32.1.7__
...
## Exercise 01: Troubleshoot `Wireless-32` wifi so that it becomes operable in the network.
...
### 10. Assign the DHCP server for the WLAN, `10.32.10.4`, finally `Apply to Device`.
...
### 13. Set the Name and Description to `AP-WLANs`. Then, under WLAN-POLICY Maps, `Add` a mapping and set the WLAN Profile to `Wireless-32` with a Policy Profile `Wireless-POL`. Then, `Apply to Device`.
...
conf t
 ...
 int vlan 11
  ...
  ip add 10.32.11.4 255.255.255.0
  ip ospf 1 area 0
  no shut
 !
 ip dhcp excluded-address 10.32.11.1 10.32.11.100
 ip dhcp pool SOCPOOL
  network 10.32.11.0 255.255.255.0
  default-router 10.32.11.4 255.255.255.0
...
### Exercise 03: Create WLANs for `THREAT-HUNTERS` on VLAN 12. Using the network address 10.32.12.0/24
...
conf t
 ...
 int vlan 12
  ...
  ip add 10.32.12.4 255.255.255.0
  ip ospf 1 area 0
  no shut
 !
 ip dhcp excluded-address 10.32.12.1 10.32.12.100
 ip dhcp pool THUNTPOOL
  network 10.32.12.0 255.255.255.0
  default-router 10.32.12.4 255.255.255.0
